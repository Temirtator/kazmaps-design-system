...
async function openLive(page: Page): Promise<void> {
  await page.goto(
    "/iframe.html?viewMode=story&id=molecules-phoneinput--live&globals=brand:business;theme:light",
  );
...
test("fill() (autofill-like) with an international number switches region", async ({ page }) => {
  await openLive(page);
  const input = page.getByLabel("Номер телефона");
...
test("picker opens with mouse and keyboard, selection refocuses the input", async ({ page }) => {
  await openLive(page);
