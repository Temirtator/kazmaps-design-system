...
- `PhoneInput` (корневой и `maps`): казахстанские номера с сетевым кодом на `7`
  ...
  для регионов с кодом `7` (KZ, RU) теперь отбрасывает эту `8`, как и раньше
  делала только вставка/вклейка через `normalizeNational` — иначе `8` занимала
  первый редактируемый слот и итоговый номер получался неверным
  (`+77870712345` вместо `+77071234567`). См. issue #8.
